...
- **`frontend-agent`:** whenever **`artifacts/design_package.json`** exists for the surface and **`stitch.source_of_truth`** is **`true`** (or exports exist under **`projects/<project_slug>/design/stitch/`** for the same `feature_slug`).
...
| Signal | Action |
|--------|--------|
| Task touches **new/changed layout** in `projects/<project_slug>/app/` / `projects/<project_slug>/src/` UI | EM **must** run **`stitch_workflow`** steps **before** frontend implementation of that layout. |
    ...
    "steps": [
      {
        ...
        "responsibility": [
          ...
          "If replacing **`projects/<project_slug>/design/stitch/meta.json`** for a **different** `feature_slug` or `surface_id`, **archive** the previous `meta.json` to **`projects/<project_slug>/design/stitch/archive/<ISO8601_Z>_<previous_feature_slug>_meta.json`** (create `archive/` as needed) so older exports remain traceable on disk."
        ],
      ...
      },
      {
        ...
        "responsibility": [
          ...
          "Write projects/<project_slug>/design/stitch/meta.json (see Meta schema)",
        ...
        "paths": {
          "images": "projects/<project_slug>/design/stitch/screens/",
          "code": "projects/<project_slug>/design/stitch/code/",
          "meta": "projects/<project_slug>/design/stitch/meta.json"
        }
      },
      {
        "step": 5,
      ...
      },
      {
        ...
        "agent": "qa-agent",
        "responsibility": [
          "Mandatory **application screenshot vs `design/stitch/screens/<id>.png`** comparison per screen in **`stitch.screens[]`** (see **`.cursor/skills/testing-and-qa-standards/SKILL.md`** §4.2): same viewport, automated diff, artifacts under **`artifacts/visual/`**.",
          "Structural fidelity vs **`ui_spec.json`** (regions, order, affordances) — **`mismatches[]`** for any drift.",
          "Responsiveness at declared breakpoints when acceptance requires it.",
          "Emit **`artifacts/stitch_fidelity_report.json`** with **`per_screen[]`** + overall **`status`** (`pass` | `fail`)."
        ],
        "validation_rules": [
          "Default **zero pixel drift** vs Stitch PNG unless **`architecture-brief.json`** documents **`stitch_qa_pixel_diff`** waiver per screen.",
          "No QA **pass** without PNG-vs-app evidence for every shipped SoT screen."
        ],
        "output": { "status": "pass | fail", "diff_report": "structured per_screen[] + mismatches[]" }
      }
    ]
  }
}
...
| Path | Content |
|------|---------|
| **`projects/<project_slug>/design/stitch/screens/`** | PNG (or MCP-provided raster) per exported screen |
| **`projects/<project_slug>/design/stitch/code/`** | HTML or structured layout export per screen |
| **`projects/<project_slug>/design/stitch/meta.json`** | Index linking `project_slug`, `feature_slug`, `surface_id`, `project_id`, `screen_ids`, files on disk |
...
| **`stitch.screens`** | Array of `{ "id", "image", "code" }` with **repository-root-relative** paths for **every** id in **`screen_ids`** (PNG + HTML/code per screen), typically rooted at **`projects/<project_slug>/design/stitch/`**. Enables **`engineering-manager-agent`** to register **both** image and code paths on **`task.json`** / EM payloads without re-querying MCP. |
...
3. **Traceability:** Every frontend PR-sized slice for a SoT surface must cite **`ui_spec.json`** + **`projects/<project_slug>/design/stitch/meta.json`** (or **`design_package.json`**) in EM handoff.
4. **QA vs design:** **`qa-agent`** checks **pixel-level** parity against Stitch PNGs (§4.2 testing skill) **and** structural parity vs **`ui_spec.json`**. **Subjective** “brand feel” remains with **`ui-critic-agent`** / **`design_visual_acceptance.skill.md`** when enabled—both can run; **PNG-vs-app** comparison is **not** optional when **`stitch.source_of_truth`** is **`true`**.
...
- Multiple unversioned “mystery” Stitch baselines not reflected in project-scoped **`meta.json`** / **`design_package.json`**.
