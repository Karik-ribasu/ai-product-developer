...
**Source-of-truth mode:** when **`engineering-manager-agent`** assigns **`stitch_workflow: source_of_truth`**, you **must** run the full **`.cursor/skills/design/stitch_workflow.skill.md`** sequence (project resolve → **2–4** variants → export images/code → persist **`projects/<project_slug>/design/stitch/**`** + **`meta.json`** + update **`design_package.json`**) in addition to per-attempt discipline.
...
1. Read assigned **`projects/<project_slug>/tasks/<feature_slug>/<task_slug>/task.json`**, **`projects/<project_slug>/tasks/<feature_slug>/architecture-brief.json`** when present, **`artifacts/design_package.json`** if it exists, plus EM handoff notes.  
2. Summarize **feature description**, **target user** (if known), and **surface type** (dashboard, app shell, landing, settings, etc.).  
3. **Resolve Stitch project (reuse first):** read existing **`stitch.project_id`** for this **`feature_slug`** from **`artifacts/design_package.json`** or EM handoff; use **`list_projects` / `get_project`** to confirm. **Reuse** that project for new screens. Call **`create_project`** only when no id exists **and** EM did not forbid it—see **`.cursor/skills/design/stitch_generation.skill.md`** §1b and **`.cursor/skills/design/stitch_workflow.skill.md`** (single project per feature). If updating **`projects/<project_slug>/design/stitch/meta.json`** for a **different** `feature_slug` / `surface_id`, **archive** the prior `meta.json` per **`stitch_workflow`** persist rules before overwriting.  
...
6. **Persist (SoT):** write PNG + code files under **`projects/<project_slug>/design/stitch/screens/`** and **`projects/<project_slug>/design/stitch/code/`**, update **`projects/<project_slug>/design/stitch/meta.json`** per **`stitch_workflow.skill.md`**.  
...
- **Do not** implement product application code in project app source such as `projects/<project_slug>/app/`, `projects/<project_slug>/src/`, etc.—write Stitch exploration outputs under assigned **`projects/.../tasks/.../artifacts/`** and, when SoT applies, **`projects/<project_slug>/design/stitch/**`** per **`stitch_workflow.skill.md`**—unless a separate task assigns application implementation.  
- **Container teardown (mandatory — no exceptions):** If you start any local container or compose stack, **tear it down** before finishing and note shutdown completed.
...
Prefer **`projects/<project_slug>/tasks/<feature_slug>/<task_slug>/artifacts/design_package.json`** (create or merge) plus optional **`ui-exploration.md`** notes—never replace `task.json` or the architecture brief yourself. When SoT applies, also write/update **`projects/<project_slug>/design/stitch/meta.json`** and companion **`projects/<project_slug>/design/stitch/screens/**` / **`projects/<project_slug>/design/stitch/code/**`** files as the project-local **layout archive** for the feature.
